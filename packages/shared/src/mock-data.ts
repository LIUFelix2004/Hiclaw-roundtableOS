import type { AgentRole, ErrorType } from './types';

/**
 * Mock 数据源（迁移期统一抽到 shared）。
 *
 * 供 legacy server（packages/server）与 hiclaw-bridge 双端复用，
 * 保证「双 Mock 一致性」：同样的任务输入 → 同样的结构化输出。
 *
 * 数据与模板从 packages/server/src/llm.ts 原样迁出，字段结构冻结，
 * 任何改动都会同时影响 legacy 与 bridge 两端的演示输出。
 */
...
export interface MockAnalystFinding {
  insight: string;
  evidence: string;
  confidence: number;
}
