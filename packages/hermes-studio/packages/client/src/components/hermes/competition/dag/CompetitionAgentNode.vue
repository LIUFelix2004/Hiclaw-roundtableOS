...
const statusLabel = computed(() => ({ pending: '待处理', running: '运行中', success: '成功', failed: '失败', rollback: '回滚' } as const)[task.value?.status || 'pending'])
</script>
<template>
  <div class="competition-node" :class="[`status-${task?.status || status?.status || 'pending'}`, { compact: props.compact }]" @click="dag.selectedTaskId = props.data.taskId">
    ...
    <div class="node-head"><span class="node-agent">{{ props.data.agent }}</span><NTag size="tiny" :type="task?.status === 'success' ? 'success' : task?.status === 'failed' ? 'error' : 'info'">{{ statusLabel }}</NTag></div>
...
.competition-node:hover { transform: translateY(-2px); border-color: var(--text-primary); }
...
.node-agent { font-size:11px; font-weight:700; text-transform:uppercase; color:var(--text-primary); }
