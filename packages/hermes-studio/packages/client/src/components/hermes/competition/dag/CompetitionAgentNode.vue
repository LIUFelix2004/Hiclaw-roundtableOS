...
<template>
  <div class="competition-node" :class="[`status-${task?.status || status?.status || 'pending'}`, { compact: props.compact }]" @click="dag.selectedTaskId = props.data.taskId">
    ...
    <div class="node-head"><span class="node-agent">{{ props.data.agent }}</span><NTag size="tiny" :type="task?.status === 'success' ? 'success' : task?.status === 'failed' ? 'error' : 'info'">{{ task?.status || 'pending' }}</NTag></div>
...
.competition-node:hover { transform: translateY(-2px); border-color: #0ea5e9; }
...
.node-agent { font-size:11px; font-weight:700; text-transform:uppercase; color:#0ea5e9; }
