...
<template><div class="topic-input"><NInput v-model:value="topic" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" placeholder="Enter a topic for the AI roundtable" @keyup.ctrl.enter="submit" /><div class="topic-controls"><NSelect v-model:value="agents" multiple :options="options" /><NInputNumber v-model:value="maxRounds" :min="1" :max="8" /><NButton type="primary" :disabled="!topic.trim()" @click="submit">Start roundtable</NButton></div></div></template>
