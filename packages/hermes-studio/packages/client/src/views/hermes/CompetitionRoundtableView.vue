...
<template><main class="competition-view roundtable-view"><header class="view-header"><div><span class="eyebrow">AGENTOS</span><h1>AI Roundtable</h1><p>Parallel agents debate, challenge, and synthesize one answer.</p></div><NTag :type="roundtable.isRunning ? 'warning' : 'default'">{{ roundtable.isRunning ? `Round ${roundtable.activeRound}` : 'Ready' }}</NTag></header><TopicInput @start="start" /><div v-if="roundtable.isRunning && !orderedSpeeches.length" class="loading"><NSpin size="small" /> Waiting for the first speech</div><section class="speeches"><SpeechCard v-for="speech in orderedSpeeches" :key="`${speech.round}-${speech.agent}-${speech.content.slice(0, 12)}`" :speech="speech" /></section><ConsensusCard v-if="roundtable.consensus" :consensus="roundtable.consensus" /></main></template>
