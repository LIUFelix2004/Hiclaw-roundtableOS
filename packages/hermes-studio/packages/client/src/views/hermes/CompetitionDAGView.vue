...
const edges = computed<Edge[]>(() => dag.tasks.flatMap(task => task.dependsOn.map(parent => ({ id: `${parent}-${task.id}`, source: parent, target: task.id, animated: task.status === 'running', style: { stroke: '#0ea5e9' } }))))
...
<template>
  <main class="competition-view dag-view">
    <header class="view-header"><div><span class="eyebrow">AGENTOS</span><h1>Execution DAG</h1><p>Live multi-agent task orchestration</p></div><NTag type="info">{{ dag.overallProgress }}% complete</NTag></header>
    <NAlert v-if="!dag.tasks.length" type="info" :show-icon="false">Start a competition chat run to populate the execution graph.</NAlert>
    ...
    <NEmpty v-if="!dag.tasks.length" description="No active task plan" />
  </main>
</template>
<style scoped>
...
.eyebrow { color:#0ea5e9; font-size:11px; font-weight:700; letter-spacing:.12em; }
h1 { margin:4px 0; font-size:24px; color:var(--text-primary); }
p { margin:0; color:var(--text-secondary); }
.flow-shell { flex:1; min-height:420px; border:1px solid var(--border-color); border-radius:8px; overflow:hidden; background:color-mix(in srgb, var(--bg-card, #fff) 94%, #0ea5e9); }
