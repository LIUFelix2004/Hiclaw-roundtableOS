...
export class ChatRunSocket {
  ...
  private onConnection(socket: Socket) {
    ...
    socket.on('run', async (data: {
      ...
      console.log('[chat-run-socket] *** run event received ***', typeof data.input === 'string' ? data.input.slice(0, 80) : 'content-blocks', 'sid=', data.session_id)
  ...
  private async handleRun(
    ...
    // Competition mode: bypass Agent Bridge, forward to B's backend
    console.log('[chat-run-socket] handleRun called, checking competition mode...')
    if (isCompetitionMode()) {
      console.log('[chat-run-socket] competition mode active, forwarding to backend')
      await handleCompetitionRun(socket, data, this.sessionMap, this.emitToSession.bind(this))
      return
    }
