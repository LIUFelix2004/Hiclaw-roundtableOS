...
/**
 * Planner（T3.1）：从 legacy packages/server/src/planner.ts 迁入。
 *
 * 与 legacy 唯一差异：LLM 走 hiclaw AI 网关（gatewayChat），
 * 模型名走网关侧映射（deepseek-chat → deepseek-v4-pro）；
 * 规则拆解、parsePlannerPlan 校验逻辑保持 1:1 一致。
 */
...
export class Planner {
  ...
  async plan(message: string, context = ''): Promise<PlanResult> {
    if (!isGatewayMockEnabled() && process.env.PLANNER_LLM !== '0') {
      try {
        const result = await gatewayChat({
          model: process.env.HICLAW_PLANNER_MODEL || 'deepseek-v4-pro',
